...
define mv-modules
    ...
    if [ "$$mdpath" != "" ];then\
        ...
        for i in $$ko; do $(ARM_EABI_TOOLCHAIN)/arm-cortex_a9-linux-gnueabi-strip --strip-unneeded $$i;\
...
ifeq ($(TARGET_ARCH),arm)
    ...
    ifneq ($(TARGET_KERNEL_CUSTOM_TOOLCHAIN),)
        ifeq ($(HOST_OS),darwin)
            ARM_CROSS_COMPILE:=CROSS_COMPILE="$(ccache) $(ANDROID_BUILD_TOP)/prebuilts/gcc/darwin-x86/arm/$(TARGET_KERNEL_CUSTOM_TOOLCHAIN)/bin/arm-cortex_a9-linux-gnueabi-"
        else
            ARM_CROSS_COMPILE:=CROSS_COMPILE="$(ccache) $(ANDROID_BUILD_TOP)/prebuilts/gcc/linux-x86/arm/$(TARGET_KERNEL_CUSTOM_TOOLCHAIN)/bin/arm-cortex_a9-linux-gnueabi-"
        endif
    else
        ARM_CROSS_COMPILE:=CROSS_COMPILE="$(ccache) $(ARM_EABI_TOOLCHAIN)/arm-cortex_a9-linux-gnueabi-"
    endif
    ccache = 
endif
